...
$(document).ready(function() {
	...
	var wd;
	 $.ajax({
			url : getRootPath()+"/nhfxCon/getHrzXx.action", 
			async : false,
			dataType : "json",
			data : {
			"hrz":$('#hrz').val(),
			...
			success : function(data) {
				...
				if(data.list.length>0){
					hrz=data.list[0].hrz;
					kxs=data.list[0].Kxs;
					grmj=data.list[0].sjmj;
				}
				
			}
		});
	 $.ajax({
			url:getRootPath()+"/nhfxCon/findryjk.action",
			async:false,
			dataType:"json",
			data:{	
				"hrz":selHrz+".读数据.",
	 ...
	 var ygrl=kxs*(18-wd)*grmj/25*24;
	     ygrl = ygrl.toFixed(2); // 输出结果为 2.45
